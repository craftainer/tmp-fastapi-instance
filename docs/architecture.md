...
and `mqtt_use_tls`/`mqtt_username`/`mqtt_password` must all be set (even
though this stack has no `mqtt` service — nothing here reaches the
broker, since `MQTTEventSink`/`MQTTEventSource` connect lazily per
publish/subscribe and `health/registry.py` registers no MQTT check, so
`/health/live`/`/health/ready` never touch it; these are just
smoke-test-only values to satisfy the guardrail). Postgres and S3 get
smoke-test-only credential overrides; Keycloak, Redis, and S3 all get a
single self-signed cert with SANs for all three in-network hostnames
...
(httpx, redis-py) and `AWS_CA_BUNDLE` (boto3, which doesn't honor
...
```mermaid
graph LR
    certgen[smoke-tls-cert] -->|writes cert/key to volume| keycloak[Keycloak]
    certgen -->|writes cert/key to volume| redis[(Redis)]
    certgen -->|writes cert/key to volume| s3[(RustFS / S3)]
    api -->|depends_on: service_healthy| postgres[(Postgres)]
    api -->|depends_on: service_healthy| s3
    api -->|depends_on: service_healthy| redis
    api -->|depends_on: service_healthy| keycloak
    api -->|trusts cert via SSL_CERT_FILE/AWS_CA_BUNDLE| certgen
```
