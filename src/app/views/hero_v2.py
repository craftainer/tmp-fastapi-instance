...
class HeroV2Update(ORMView):
    ...
    crud.controllers.crud_router's `/draft` route and app/README.md's "Example
    ...
    dedicated action) -- see crud.models.mixins.Lockable/Schedulable: lock/unlock
...
class HeroV2(ORMView):
    ...
    draft Hero -- see crud.models.mixins.Draftable -- may have either or both
    still unset. `is_draft`/`archived_at`/`publish_at`/`unpublish_at`/`is_locked`
    are every record-lifecycle mixin Hero demonstrates (see crud.models.mixins);
